class Factory {
  ...
  injectHeader = () => {
    let identityComponent = this.getComponent.createComponents(
      new createHeader()
    );
  ...
  injectIdentities = () => {
    ...
    let identityComponent = this.getComponent.createComponents(
      new createSticker(dataJSON)
    );
    this.getComponent.renderComponent(
      identityComponent,
      this.targetMainWrapper
    );
  };
  ...
  injectIdentity = () => {
    ...
    const identityComponent = this.getComponent.createComponents(
      new createSticker(dataJSON)
    );
  ...
  injectCustomSelect = () => {
    ...
    const customSelectComponent = this.getComponent.createComponents(
      new CreateCustomSelect()
    );
    this.getComponent.renderComponent(
  ...
  injectMedia = () => {
    ...
    let mediaComponent = this.getComponent.createComponents(
      new createMedia(dataJSON)
    );
  ...
  injectCounter = () => {
    ...
    let counterComponent = this.getComponent.createComponents(
      new createCounter(dataJSON)
    );
    this.getComponent.renderComponent(counterComponent, this.targetRoot);
  };

  injectContactBtn = () => {
    let contactBtnComponent = this.getComponent.createComponents(
      new CreateContactBtn()
    );
    this.getComponent.renderComponent(contactBtnComponent, this.targetMain);
  };

  injectModalForm = () => {
    let modalFormComponent = this.getComponent.createComponents(
      new createModalForm()
    );
  ...
  injectCarousel = () => {
    let dataJSON = GetData.getMediaByIdentity();
    let carouselComponent = this.getComponent.createComponents(
      new createCarousel(dataJSON)
    );
  ...
  injectGallery() {
    document
      ...
      .classList.remove("main__wrapper--index");
    document
      .querySelector(".main__wrapper")
      .classList.toggle("main__wrapper--gallery");
