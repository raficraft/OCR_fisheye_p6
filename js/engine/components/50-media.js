    ...
    thisData.forEach((media, key) => {
      ...
      this.elHTML += `
        <article class="sticker__media ${thisClass}" >             
                          <a href="#" class="flexImg"  data-js="openCarousel" data-item="${key}" role="button">`;
      ...
      if (media.video) {
        ...
        this.elHTML += `<video class="imgFlex ${
          ...
          media.video
                  ...
                  <span class="sr__only">Cette vidéo ce nomme ${ucFirst(
                    media.alt
                  )},
                  a un prix de ${media.price} euro et a actuellement eu ${
          media.likes
      ...
      <article class="sticker__media--info">   
        ...
        <p class="sticker__media--price"  aria-hidden="true">${
          media.price
        } €</p>
        <a class="sticker__media--like" data-js="addLike" data-status="true" tabindex="0" aria-label="likes" role="button">${
          media.likes
        }
        <i class="fas fa-heart fontIcon" aria-label="likes"></i></a>
